...
from src.async_task_pipeline.base.pipeline import AsyncTaskPipeline
from src.async_task_pipeline.utils import log_pipeline_performance_analysis, logger
...
if __name__ == "__main__":
    ...
    def simulate_cpu_intensive_task(name: str, processing_time: float, cpu_intensity: int = 1000):
    ...
    async def example_output_consumer(output_stream: AsyncIterator[Any]):
    ...
    async def main(args):
        ...
        tasks = [
            asyncio.create_task(
                pipeline.process_input_stream(example_input_stream(50, 0.01))
            ),
            asyncio.create_task(
                example_output_consumer(pipeline.generate_output_stream())
            )
        ]
